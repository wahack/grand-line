...
export class PetOwner extends Entity {
  ...
  save(): void {
    let id = this.get("id");
    assert(id != null, "Cannot save PetOwner entity without an ID");
    if (id) {
      assert(
        id.kind == ValueKind.BYTES,
        `Entities of type PetOwner must have an ID of type Bytes but the id '${id.displayData()}' is of type ${id.displayKind()}`,
      );
      store.set("PetOwner", id.toBytes().toHexString(), this);
    }
  }

  static loadInBlock(id: Bytes): PetOwner | null {
    return changetype<PetOwner | null>(
      store.get_in_block("PetOwner", id.toHexString()),
    );
  }

  static load(id: Bytes): PetOwner | null {
    return changetype<PetOwner | null>(store.get("PetOwner", id.toHexString()));
  }
